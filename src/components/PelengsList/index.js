          ...
          return (
            <Toast key={peleng.id}>
              <Toast.Header closeButton={false}>
                ...
                <strong className='me-auto' style={{ color: peleng.color }}>{peleng.peleng}<sup>o</sup></strong>
                <CustomButton iconPath={`${process.env.PUBLIC_URL}/images/icons/delete.png`}/>
              </Toast.Header>
              <Toast.Body>
                <strong>Координати точки відліку:</strong>
                <p>Ш. <i>(квадратичні координати)</i>: {peleng.lat.toFixed(8)}</p> 
                <p>Д. <i>(квадратичні координати)</i>: {peleng.lng.toFixed(8)}</p> 
              </Toast.Body>
            </Toast>
          )
        })
