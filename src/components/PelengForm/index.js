...
import { getPelengsAmount, getCurrentPeleng } from '../../selectors';
import { addPelengToDraw } from '../../reducers/peleng';

const initialValues = Object.fromEntries(Object.entries(pelengFormConfig).map(([key, type]) => [key, type === 'number' ? 0 : '']));
...
export function PelengForm() {
  ...
  const { values, handleChange, handleSubmit, errors, touched, handleBlur, setFieldValue, setValues, setFieldTouched } = useFormik({
    initialValues,
    validationSchema,
    onSubmit: onSubmit
  });
  ...
  useEffect(() => {
    if (currentPeleng.lat && currentPeleng.lng) {
        setFieldValue(pelengFormConfig.startPelengPoint.fieldName, `${currentPeleng.lat}, ${currentPeleng.lng}`);
    }
  }, [currentPeleng])
  ...
  const renderForm = useCallback(() => {
    return (
        <>
            {
                Object.entries(pelengFormConfig)
                    .map(([ name, fieldProps ]) => (
                        <FormField 
                        key={name}
